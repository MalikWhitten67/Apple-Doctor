...
export default function Register(){
  ...
  function register(e) {
    ...
    authData.catch((err) => {
        ...
        if(!data) return;
         ...
         if(err.data.code == 400 && data.email && data.email.code === 'validation_invalid_email'){
            setError({message: data.email.message == 'The email is invalid or in use.'? 'Email is already in use!' : 'Invalid Email'})
            document.querySelector('input[type="email"]').focus()
         }else if (err.data.code == 400 && data.passwordConfirm && data.passwordConfirm.code === 'validation_required'){
            setError({message: `Password Confirm is required`})
         }else if (err.data.code == 400 && data.email && data.email.code === 'validation_is_email'){
 ...
 return (
     <div>
      ...
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6  text-error h-6">
  <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
    ...
    <div className="hero  w-screen  mt-16 justify-center flex flex-col gap-5 mx-auto">
        ...
        <div className=" mb-8 ">
            ...
            font-bold  mx-auto  w-[85vw] justify-center`}>
                Join <span className='text-blue-500'>Apple Doc</span> -
                 To enhance your  {isDoctor ? `patient's` : ``} experience!
            </h1>
        </div>
        ...
        <a href='/login' className="link text-md">Already have an account? Login</a>
        </div>
